...
export class OrdemItem extends Item {
	...
	async upgradeDie(die, steps = 1) {
		const dieSteps = [
			"1", "1d2", "1d3", "1d4", "1d6", "1d8", "1d10", "1d12",
			"2d8", "2d10", "2d12", "3d10", "3d12", "4d10", "4d12"
		];
		...
		if (newIndex >= dieSteps.length) {
			const additionalSteps = newIndex - (dieSteps.length - 1);
			return `4d12+${additionalSteps}d6`;
		} else {
			return dieSteps[newIndex];
		}
	}
	
	async getMaxDamage(formula) {
		let maxDamage = 0;
		
		const split1 = formula.trim().split('+');
		
		for (let f of split1) {
			const [num, die] = f.trim().split('d');
			maxDamage += (num * die);
		}
	...
	async rollDamage(options = {}) {
		...
		if ((critical.isCritical && options.lastId) || options.event.altKey) {
			...
			const upgradedDie = await this.upgradeDie(damage.formula, critical.multiplier-2);
			prepareFormula.push(upgradedDie);
			prepareFormula.push(await this.getMaxDamage(upgradedDie));
		} else {
